document.addEventListener('DOMContentLoaded', function () {
    ...
    const fetchHeader = fetch('header.html')
        ...
        .then(content => {
            if (isIndex) {
                // On index.html, internal links should be "#section" not "./#section"
                content = content.replace(/href="\.\/(\#contact|\#about|\#learn)"/g, 'href="$1"');
                // Fix main "Join Us" CTA
                content = content.replace(/class="cta main-cta" href="\.\/#contact"/g, 'class="cta main-cta" href="#contact"');
            }
            const headerPlaceholder = document.getElementById('header-placeholder');
            if (headerPlaceholder) headerPlaceholder.innerHTML = content;
        });
    ...
    const fetchFooter = fetch('footer.html')
        ...
        .then(content => {
            ...
            if (footerPlaceholder) footerPlaceholder.innerHTML = content;
        });

    // Wait for both header and footer to load before initializing scripts
    Promise.all([fetchHeader, fetchFooter])
        .then(() => {
            initializeApp(); // Call the original script logic
        })
        .catch(error => {
            console.error('Error loading components:', error);
            initializeApp(); // Still try to init, maybe some parts will work
        });
    // --- End of new code ---


    // --- All original code is wrapped in this function ---
    function initializeApp() {
        ...
        // Add checks for all header/footer elements
        if (logo) {
        ...
        function loadEventsFromTxt(startIndex = 0, loadAmount = 4) {
            fetch('static/events.txt')
                ...
                .then(text => {
                    ...
                    if (!eventsContainer) return; // Exit if container not on page
                    ...
                    if (loadMoreButton && endIndex >= events.length) {
        ...
        const bibLinks = document.querySelectorAll('.bib-link');
        const modalOverlay = document.getElementById('bib-modal-overlay');
        const closeButtons = document.querySelectorAll('.bib-modal-close');
        ...
        function closeModal() {
            const allModals = document.querySelectorAll('.bib-modal.visible');
            if (modalOverlay) {
        ...
        // Add click listeners to all [BIB] links
        ...
        // Add click listeners to all 'X' close buttons
        ...
        // --- END OF BIB MODAL LOGIC ---


        // The event loading logic was commented out in the original file, so keeping it that way.
        // // Load initial events
        // let loadedEvents = 0;
